...
public class CourseService {
    ...
    public CourseResponse updateUploadStatus(UUID courseId,
        ...
        if (newStatus == UploadStatus.READY) {
            ...
            // If no real video URL set (local dev), use demo video so player works
            if (course.getVideoUrl() == null || course.getVideoUrl().isEmpty()
                    || !course.getVideoUrl().startsWith("http")) {
                course.setVideoUrl(demoVideoUrl);
                log.info("Local mode — set demo video URL for course: {}", courseId);
            }
        }
